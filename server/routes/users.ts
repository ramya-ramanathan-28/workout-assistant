...
const fetchBuckets = async (buckets: number = 3): Promise<any[]> => {
  let clusterListresponse: Array<any> = await new Promise(
    (resolver, errhandler) => {
      kmeans.clusterize(featuresList, { k: buckets }, (err, res) => {
        if (err) console.error(err);
          ...
          res.forEach((cluster: any, index: number) => {
            centroid.push({
              tempo: cluster.centroid[0],
              danceability: cluster.centroid[1],
              energy: cluster.centroid[2],
              centroid: index,
            });
            clusterList.push({
          ...
          centroid.sort((a, b) =>
            a.tempo + a.danceability + a.energy >
            b.tempo + b.danceability + b.energy
              ? 1
              : -1
          );
      ...
      if (bucket.intensity === songIntensity - 1) {
        ...
        while (slotTimeRemaining >= -30000 && songs.length) {
          songs = songs.filter((song: any) => {
            return (
              song['duration'] <= slotTimeRemaining + 30000 &&
              !visited.includes(song['id'])
            );
          });
          if (songs.length) {
